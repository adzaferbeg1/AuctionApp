...
export default function Register() {
  ...
  const validateInput = (name, surname, email, password) => {
    if (name.length < 2 || surname.length < 2) return 'Name or surname must contain 2 or more characters';
    ...
    <div className='reg-login'>
      ...
      <Form className="register-form" onSubmit={submitButton}>
        ...
        <Form.Group >
          ...
          <Form.Control style={registerInput} type="text" value={name} onChange={e => setName(e.target.value)} name="name" required />

        </Form.Group>
        <Form.Group >
          <Form.Label className="reg-label">Last Name</Form.Label>
          <Form.Control style={registerInput} type="text" value={surname} onChange={e => setSurname(e.target.value)} name="surname" required />

        </Form.Group>
        <Form.Group >
          <Form.Label className="reg-label">Enter Email</Form.Label>
          <Form.Control style={registerInput} type="email" value={email} onChange={e => setEmail(e.target.value)} name="email" required />

        </Form.Group>
        <Form.Group >
          <Form.Label className="reg-label">Password</Form.Label>
          <Form.Control style={registerInput} type="password" value={password} onChange={e => setPassword(e.target.value)} name="password" required />
